...
DecayLevel& d3 = *quiver.AddLevel("d3");
...
DecayTransition& gamma32 = *quiver.AddTransition(
    "gamma32",
    &d3,
    &d2,
    0.5
...
PathAlgebra pathAlgebra(quiver);
assert(&pathAlgebra.GetQuiver() == &quiver);
assert(pathAlgebra.MaxPower() == 3);
const DecayVector identity = pathAlgebra.Identity();
assert(identity.Size() == 4);
for (auto* level : quiver.GetLevels()) {
    const DecayVector vertex(DecayPath(level), 1.0);
    const auto projected = pathAlgebra.Multiply(identity, vertex);
    assert(projected.Size() == 1);
    assert(projected.GetTerms()[0].path == DecayPath(level));
    assert(std::abs(projected.GetTerms()[0].coefficient - 1.0) < 1.0e-12);
}
